    ...
    if (OPENAI_API_KEY === 'your_openai_api_key_here' || OPENAI_API_KEY === '') {
      ...
      const mockData: AIEventResponse = {
        event1: {
          ...
          goal: `브랜드 인지도 상승과 사용자 참여 증대를 통해 ${eventData.productName}의 가치를 널리 알리고, MZ세대를 중심으로 한 고객층 확대를 목표로 합니다. 정량적으로는 팔로워 20% 증가, 참여 게시물 500개 이상, 해시태그 노출 50,000회 이상을 달성하며, 정성적으로는 브랜드에 대한 긍정적 인식 증진과 고객과의 정서적 유대감 강화를 추구합니다.`,
          performanceMetric: `주요 KPI인 ${eventData.kpiMetrics.join(', ')}를 주간 단위로 모니터링하며, 참여율 10% 미달 시 인플루언서 추가 투입, 15% 초과 시 이벤트 기간 연장을 검토합니다.`,
          rewards: "1등(1명): 제품 풀세트 30만원 상당, 2등(3명): 신제품 패키지 15만원 상당, 3등(6명): 미니 샘플러 세트 5만원 상당, 당첨자 발표 후 개별 연락하여 배송 정보 수집",
